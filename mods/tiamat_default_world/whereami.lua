...
function tdw.seek_landed(uuid, rec, x, y, z)
    ...
    if tdw.biome_under(x, y, z) == seeking.id then
        rec.seeking = nil
        game.log(string.format("tiamat_default_world: %s found %s by trial at %d, %d (try %d)", uuid, biome.name, x, z, seeking.at))
        return true
    end
    ...
    if place == nil then
        rec.seeking = nil
        game.log(string.format("tiamat_default_world: %s did not find %s by trial", uuid, biome.name))
        return true
    end
