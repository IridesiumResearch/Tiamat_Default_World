...
local GRANITE, SLATE = "tiamat_default_world:granite", "tiamat_default_world:slate"
local FIR_LOG, FIR_NEEDLES = "tiamat_default_world:fir_log", "tiamat_default_world:fir_needles"
local DEAD = "tiamat_default_world:dead_log"
...
local function carve_hollow(x, y, z, rng)
    ...
    for _, s in ipairs(spheres) do
        schem.push_ellipsoid("tiamat_default_world:ice", s[1], s[2], s[3], s[4] + HOLLOW_CRUST, s[4] * 0.8 + HOLLOW_CRUST, s[4] + HOLLOW_CRUST,
...
local function carve_crevasse(x, y, z, rng, anywhere)
    ...
    edits.begin()
    schem.push_ellipsoid("tiamat_default_world:ice", cx, cy, cz, rx + coat, depth * 0.5 + coat, rz + coat,
...
tdw.on_command("crevasse", "/crevasse — crack a crevasse open under you", function(player)
    ...
    game.log(string.format("tiamat_default_world alpine: crevasse at %d, %d, %d: %s", x, y, z, ok and "carved" or "refused"))
...
tdw.on_tick(function(dt_ticks)
    ...
    game.log(string.format(
        "tiamat_default_world alpine: %d turns (%d buried ticks taken up, %d lost), %d firs of %d, %d boulders of %d, %d rocks of %d, %d crevasses of %d, %d hollows of %d (no wall %d, room %d, unloaded %d); refused: flat %d, headroom %d, spacing %d, room %d, unloaded %d; errors %d (%s)",
...
if game.schematic then
    ...
    game.log(string.format("tiamat_default_world alpine: %d fir schematics, %d blocks, for the scatter", #FIR_SCHEMATICS, blocks_total))
else
    game.log("tiamat_default_world alpine: no game.schematic in this engine; the firs grow by tick alone")
end
