...
local function fills_of(id)
    ...
    if entry.fills == nil then
        ...
        if not ok then
            game.log("tiamat_default_world: building the cave " .. id .. " failed: " .. tostring(fills))
            error(fills, 0)
        end
        entry.fills = fills
        ...
        game.log(string.format("tiamat_default_world cave %-20s compiled %d fill(s)", id, #fills))
    end
    return entry.fills
end
M.fills_of = fills_of
...
function M.into(buf, pos, dmin, dmax)
    ...
    if stats.chunks % 512 == 0 then
        game.log(string.format("tiamat_default_world caves: %d chunks, %d carves, %d structures", stats.chunks, stats.carved, stats.stamped))
    end
end
