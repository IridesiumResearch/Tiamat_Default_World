...
local LOG, LEAVES = "tiamat_default_world:oak_log", "tiamat_default_world:oak_leaves"
...
local function place_erratic(x, y, z, rng)
    ...
    edits.begin()
    tdw.rocks.place_rock("tiamat_default_world:granite", x, surface, z, r, rng,
...
local ROSE_BUSH, ROSE_BLOOMS_ID = "tiamat_default_world:rose_bush", "tiamat_default_world:rose_blooms"
...
tdw.on_command("roses", "/roses — go to the first rose bush that grew this session", function(player)
    ...
    game.log(string.format("tiamat_default_world grasslands: first rose bush at %d, %d, %d", first_bush.x, first_bush.y, first_bush.z))
...
tdw.on_random_tick(blocks.grass, function(x, y, z)
    ...
    if not ok then
        ...
        if last_error ~= tostring(err) then
            ...
            game.log("tiamat_default_world grasslands: grass tick failed: " .. last_error)
        end
    end
    return true
end)
...
tdw.on_tick(function(dt_ticks)
    ...
    game.log(string.format(
        "tiamat_default_world grasslands: %d grass turns, %d tries: %d sentinels of %d, %d burrows of %d, %d erratics, %d rose bushes of %d, %d picked; refused: spacing %d, headroom %d, flat %d, shallow %d, room %d, unloaded %d; errors %d (%s)",
