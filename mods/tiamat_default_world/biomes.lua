...
function tdw.build_biome(id, build)
    ...
    local function compile_fills(mode)
        ...
        if not ok then
            game.log("tiamat_default_world: building the fills of " .. id .. " in mode " .. tostring(mode) .. " failed: " .. tostring(fills))
            error(fills, 0)
        end
    ...
    if only and only ~= id then
        game.log(string.format("tiamat_default_world biome %-24s built, fills not compiled (%s is everywhere)", id, only))
        return
    end
    if biome.lazy then
        game.log(string.format("tiamat_default_world biome %-24s built lazily, mode %s", id, own))
        return
    end
    biome.fills = compile_fills(own)
    game.log(string.format("tiamat_default_world biome %-24s built, %d fill(s), mode %s", id, #biome.fills, own))
end
...
function tdw.fills_for(biome, mode)
    ...
    if set == nil then
        if biome.fills and mode == biome.own_mode then
            ...
            game.log(string.format("tiamat_default_world biome %-24s compiled %d fill(s), mode %s", biome.id, #set, mode))
        end
