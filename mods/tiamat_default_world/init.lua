...
local function load(name)
    ...
    if not ok then
        game.log(string.format("tiamat_default_world: %s.lua failed: %s", name, tostring(result)))
        error(result, 0)
    end
    return result
end
...
game.log(string.format("tiamat_default_world ready: %d areas, %d biomes (%d built)",
